  ...
  return (
    <div className="w-full full flex items-center gap-3 overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 text-playerColor">
          <AudioPlayer
        ...
        <div className="h-full flex items-center justify-center flex-col gap-3">
          <motion.i whileTap={{ scale: 0.8 }} onClick={closeMusicPlayer}>
            <IoMdClose className=" hover:text-headingColor text-2xl cursor-pointer" />
          </motion.i>
      ...
      {miniPlayer && (
        ...
        >
          <div className="w-40 h-40 rounded-full flex items-center justify-center  relative ">
            <div className="absolute inset-0 rounded-full bg-red-600 blur-xl animate-pulse text-playerColor"></div>
            <img
      ...
      {allSongs.length > 0 ? (
        allSongs.map((music, index) => (
          <motion.div
            ...
            className={`group w-full p-4 hover:bg-card flex gap-3 items-center cursor-pointer text-playerColor ${
              music?._id === song._id ? "bg-card" : "bg-transparent"
            }`}
            onClick={() => setCurrentPlaySong(index)}
          >
            <IoMusicalNote className="text-textColor text-playerColor group-hover:text-headingColor text-2xl cursor-pointer" />

            <div className="flex items-start flex-col">
              <p className="text-lg text-headingColor font-semibold text-playerColor">
                {`${
              ...
              <p className="text-textColor">
                {music?.artist}{" "}
                <span className="text-sm text-textColor font-semibold text-playerColor">
                  ({music?.category})
                </span>
              </p>
            </div>
